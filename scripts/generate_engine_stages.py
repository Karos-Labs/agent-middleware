...
def stages_for_workflow(
    ...
    for _pos, kind, raw_id in sorted(calls):
        ...
        if kind == "agent":
            ...
            if policy:
                entry.update(policy)
        # First writer wins, but a later one carrying a skillRef or a resolved
        # class upgrades it.
        if step_id in found and "skill_ref" not in found[step_id] and "skill_ref" in entry:
            found[step_id] = entry
        elif step_id in found and "agent_id" not in found[step_id] and "agent_id" in entry:
            found[step_id] = {**found[step_id], **entry}
